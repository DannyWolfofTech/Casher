-- Authenticated, owner-scoped corrections with immutable change history.
CREATE TABLE public.statement_reviews (
  id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  record_kind text NOT NULL CHECK (record_kind IN ('transaction', 'subscription')),
  record_id uuid NOT NULL,
  previous_values jsonb NOT NULL,
  new_values jsonb NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX statement_reviews_owner_idx ON public.statement_reviews(user_id, created_at DESC);
ALTER TABLE public.statement_reviews ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Owners can read their correction history" ON public.statement_reviews FOR SELECT TO authenticated USING (user_id = auth.uid());
GRANT SELECT ON public.statement_reviews TO authenticated;
REVOKE INSERT, UPDATE, DELETE ON public.statement_reviews FROM anon, authenticated;
...
BEGIN
  ...
  IF v_row.reviewed_at IS DISTINCT FROM _expected_reviewed_at THEN RAISE EXCEPTION 'This record has changed. Reload and try again.' USING ERRCODE = '40001'; END IF;
