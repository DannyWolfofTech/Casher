...
The Android app is a signed release using bundled production assets, installed and tested on a dedicated Android 16 emulator. iOS now compiles and passes three real UI tests as a Release simulator app on GitHub's standard macOS runner. Neither app has been submitted to a store. Simulator acceptance does not establish signed-device acceptance.

## Implemented

Latest Android refresh: application source `9685c91`, including the Zoho correspondence disclosure. Release assembly/bundle/lint/unit tasks passed and the unchanged signing certificate verified; the APK installed successfully on the existing emulator. Current APK SHA-256 is `24E8A7A8061C3E31154740E9D693200357596EA50013791BD5A6F00A04301CAC` (4,307,292 bytes). AAB SHA-256 is `E12C8D2EA14A715B3D4325D788224CEEC073A3DBC565EC1CF45C907BDA19F106` (4,194,184 bytes). Paths below are unchanged.

Latest iOS refresh: [run 34161059742](https://github.com/DannyWolfofTech/Casher/actions/runs/34161059742) at `9685c91` passed the iPhone SDK and simulator Release builds, then all three UI tests (106.518 seconds, zero failures). Both downloaded packages passed `inspect-ios-release.py`, including production API, identity, architecture and privacy checks. Current `release-artifacts/ios/Casher-Device-Unsigned.zip` is 4,003,203 bytes, SHA-256 `1432677A2718D835A4879D06F3DCAC0C4A9972CACA18A82067F64EFD61E3670C`. Current `release-artifacts/ios/Casher-Simulator.zip` is 4,721,366 bytes, SHA-256 `D55BD2D17F4CD8A6269E18B2ACA718FBBDB25DA7AB4BFE847A4FC18466817C79`. These replace the artifacts described in the historical sections below. The unsigned device package cannot be installed on an iPhone or submitted to a store; signed-device acceptance remains open.
...
`.github/workflows/mobile-ios.yml` uses a standard macOS 26 runner for this public repository, without distribution credentials or a paid runner. It prepares only the intentionally public production configuration, runs `tools/mobile/build-ios-release.sh`, and packages `release-artifacts/ios/Casher-Simulator.zip`. The script converts the opaque Casher icon source to 1024px, syncs plugins with the committed CocoaPods lock in deployment mode, rejects lock drift, checks plists and builds the actual Release target. Preserve the ZIP when moving it from Windows to a Mac so framework symlinks survive extraction with `ditto`.

The [verified hosted release](https://github.com/DannyWolfofTech/Casher/actions/runs/34120815933) succeeded at `4c13598`. Independent inspection confirmed version `1.0.0`, `com.trycasher.app`, iOS 15 minimum, arm64 and x86_64 simulator architectures, the production API, disabled cleartext/logging/remote-server settings and no tracking. Xcode's local simulator signature supplies the Keychain identity without an Apple account or distribution certificate. The earlier unsigned version `1.0` compiled but could not use Keychain; it has been superseded. No insecure storage fallback was added.

Three XCTest UI checks passed on an iPhone 17 Pro simulator running iOS 26.2: privacy navigation, a real recovery request to production Auth through native Keychain/PKCE, and signed-out navigation/relaunch/rotation. The six screenshots were downloaded and visually reviewed. The final acceptance report records the delivered ZIP's exact source and SHA-256.
...
Final Android application source: `4c13598`. Its rebuilt signed APK/AAB include local licensed fonts and accessibility corrections. The final native run recorded 3 passes and 2 explicit skips; earlier disposable production session/file/export acceptance was at `983d59f`. Artifact hashes and the distinction between those runs are recorded in release acceptance.

## Launch-unblocking follow-up

[Hosted run 34123424104](https://github.com/DannyWolfofTech/Casher/actions/runs/34123424104) at `952ebbd97303d4ad1e69a1d36523be71c074c9a6` passed both simulator Release and the actual iPhone SDK Release build, followed by all three UI tests. `release-artifacts/ios/Casher-Device-Unsigned.zip` was downloaded and independently inspected: arm64, iPhoneOS, iOS 15 minimum, version 1.0.0, production API, no distribution profile. SHA-256: `D4484F432DBE1C907A5AA66DE1D1ACA84C1CE7C0F5598E4D8CB6A58DCAA867EA`. It is compilation evidence only and cannot be installed or submitted. The previously delivered tested simulator ZIP and Android APK remain unchanged.

The Android key and DPAPI password now have an access-restricted, hash-matched backup under the owner’s local Casher SigningBackups folder; the private key reopened and matched the release certificate. Portable password-owner recovery is still required. See [the exact physical checklist and signing steps](launch-unblocking.md).
