...
The catch-up completed: **1,611,280 expired log rows removed**, then a history-table-only `VACUUM (FULL, ANALYZE, SKIP_LOCKED)` reclaimed its unused space. At 15:40 UTC, the database measured **15,404,179 bytes** and cron history **147,456 bytes**, with **446 retained runs and zero expired runs**. The first compaction attempt skipped an active automatic vacuum; completion was verified after retrying when that worker had finished. All **29 profiles, 490 transactions and 60 detected subscriptions** remained, and live health checks passed throughout. No customer table was rewritten. [Exact maintenance evidence](recovery-maintenance-20260907.json).

Two job starts at 15:40 UTC returned `job startup timeout` at approximately 15:40:22 during compaction. Email processing resumed successfully at 15:41 and billing reconciliation at 15:45; later runs through 15:55 also succeeded. Auth mail, transactional mail and billing-reconciliation backlogs were all zero. These failure records remain in the retained history. Reserve history-table FULL compaction for a planned quiet period; its exclusive table lock can interrupt scheduled-job startup. Normal retention and routine vacuuming do not require this one-time rewrite.

`tools/release/prune-expired-cron-history.sql` is the small manual pilot batch. Larger catch-up operations used explicit run-ID ranges of at most 50,000, the same completed-before cutoff, and completion/health checks between batches. Lovable's SQL tool can return HTTP 499 while a statement is still executing: inspect active database work and committed deletion statistics before retrying. Never infer rollback from that response.

The documented 5 GB size blocker is resolved. At the enabled **Start export** confirmation, automatic approval review instead required explicit permission for the full database copy in this project's Lovable Cloud storage and the temporary download link emailed to the account owner. The export did not start. Do not bypass that transfer-approval boundary. A successful export/isolated restore is still required before claiming a tested recovery path.
...
`tools/release/recovery-baseline.sql` was executed against production. It records application/Auth row counts, all 23 application tables' RLS flags, policy/function/schema fingerprints, 34 migration versions and deletion-ledger reconciliation without exporting customer rows or secrets. The protected baseline is in `.audit-results/recovery-drill`; both recorded deleted user IDs are absent from Auth. Compare a fresh baseline with the isolated restore and exercise anonymous/authenticated isolation before reporting success. Retained daily-snapshot restoration, storage objects and authentication recovery remain separate from a fresh logical-export drill.
...
### Statement-reset receipts (11 September 2026)

Before any restore or cutover, preserve the latest private `app_private.statement_reset_receipts` journal alongside the existing account-deletion journal. It contains account/request IDs, completion time, counts and exact deleted record IDs, without statement text or amounts. Do not put either journal in Git, screenshots or public artifacts. Import current journals into the isolated recovery target and reapply account deletions first. For surviving accounts, remove only matching owner + record IDs from transactions, detected_subscriptions, statement_reviews and upload_history, then restore the receipt so a delayed client retry is idempotent. Clear legacy monthly_spending_history for those owners while preserving goals, billing and upload usage. Do not use timestamp cutoffs: an import can begin before a reset, wait on the profile lock and commit afterward with an earlier row timestamp. Verify the journal covers the outage before making restored data available; a stale backup's own journal cannot prove recent deletions were reapplied. This addition does not resolve the replacement-service recovery/cutover blockers recorded in `recovery-drill-20260907.md`.
