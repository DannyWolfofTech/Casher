...
A restore drill has not been performed. Do not restore production merely to test recovery. Before a real restore, freeze writes/jobs and export the current deletion UUID ledger into a protected operator-controlled location. Restore into an isolated project if the provider supports it, replay deletion receipts and reconcile every Stripe-bound customer before resuming access. Verify row counts, RLS, Auth behavior, migrations and export/import workflows. Provider support may be needed for an isolated restore. The owner must approve any paid recovery environment.
...
The Production health manual workflow now has `test_failure_notification`, default false. After the owner enables GitHub failure emails, enabling that input deliberately fails a clearly labelled final step after real healthy checks, to prove notification delivery. Scheduled runs never enable it. The [ordinary hosted run](https://github.com/DannyWolfofTech/Casher/actions/runs/34124262994) passed and correctly skipped the drill. Four provider jobs remain active; the recent health dispatch had no recorded error. Scheduled GitHub execution is still unverified.
