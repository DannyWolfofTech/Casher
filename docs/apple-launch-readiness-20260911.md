...
A fresh signup after deletion was rejected with `email rate limit exceeded`; no replacement account was created. Lovable browser access was restored later that evening. Cloud → Users → Auth settings → Email → Advanced showed an unset hourly email limit with the placeholder “Default value”; it did not expose the numeric inherited ceiling. Cloud → Emails confirmed the existing `trycasher.com` sending domain is Verified and displayed both iPhone authentication emails as completed. The project limit was set to an explicit **20 authentication emails per hour**, saved, and visually verified after reopening the settings. This is a conservative initial release capacity, not an unlimited or load-tested quota. Email confirmation, secure email change and HIBP protection remain enabled; no provider, plan, billing, DNS or other auth setting was changed. Lovable's current documentation distinguishes a Pro workspace limit of 500 authentication emails/hour from this project-level limit and includes 50,000 transactional emails/month. Actual workspace-wide remaining monthly usage was not exposed by the inspected domain analytics. No additional email was sent in this configuration pass. [Lovable project sending limits](https://docs.lovable.dev/features/email-auth#sending-limits), [Lovable workspace email limits](https://docs.lovable.dev/features/custom-emails#availability-and-usage).

The quota save is configuration evidence only: fresh physical confirmation and warm callback are still pending. The connected iPhone reported `passcodeRequired: true` at 22:35 BST, so the owner was asked to unlock it before another disposable native signup. The selected mailbox still had zero Casher identities. Do not attribute the previous failure to an exact two-email ceiling or claim that this change has passed an end-to-end retry yet.

Later physical retry: fresh signup passed after the quota change and the confirmation reached iCloud Inbox. Opening it in iPhone Mail confirmed the mailbox and reached the browser's Open Casher fallback while the app was closed. XCTest could not complete the browser tap; after owner intervention the app opened but reported an unsuccessful callback. **The old error text did not distinguish connection failure from an expired/rejected code, so expiry is not established as the cause.** The callback now maps retryable network/server failures to an accurate connection message that asks for a fresh link. Supabase's installed SDK removes its PKCE verifier after a failed exchange; no code replay, verifier restoration or security downgrade was introduced.

The first native password sign-in returned a connection error; the visible retry succeeded. Onboarding, exact disposable-account identity and login persistence across process termination/relaunch then passed. Fresh recovery was requested for a warm-return test. Its final handoff and fixture cleanup are tracked in `.audit-results/email-retry-20260911/RESULTS.md`; do not infer completion from the request. This pass's source validation: 263 unit tests, typecheck/build, zero lint errors with seven existing warnings, and five focused Chromium plus five WebKit callback/error cases.
