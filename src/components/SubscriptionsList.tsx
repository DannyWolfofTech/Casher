  ...
  return <Card className="min-w-0">
    ...
    <CardContent>
      {!showInactive && !query.isPending && !query.isError && <section className="mb-6 space-y-3 rounded-lg border p-3" aria-label="Expected renewals">
        ...
        <label className="block text-xs">Renewal month<select aria-label="Renewal month" value={renewalMonth} onChange={event => setRenewalMonth(event.target.value)} className="mt-1 h-11 w-full min-w-0 rounded-md border bg-background px-3 text-base">{renewalMonths.map(month => <option key={month} value={month}>{monthLabel(month)}</option>)}</select></label>
        {upcoming.length ? <ul className="divide-y">{upcoming.map(({sub, due}) => <li key={sub.id} className="flex flex-wrap justify-between gap-2 py-2 text-sm"><span>{cancellationProvider(sub.service_name)?.name || sub.service_name}<span className="block text-xs text-muted-foreground">Expected {new Date(`${due}T12:00:00`).toLocaleDateString('en-GB', {day:'numeric', month:'short', year:'numeric'})}</span></span><span>{money(Number(sub.amount))}</span></li>)}</ul> : <p className="text-sm text-muted-foreground">No renewals can be estimated for this month from your latest payments. Upload a recent statement to update them.</p>}
      ...
      {query.isPending ? <p role="status">Loading subscriptions…</p> : query.isError ? <div role="alert"><p>Subscriptions could not be loaded.</p><Button variant="outline" onClick={() => query.refetch()}>Try again</Button></div> : subscriptions.length === 0 ? <p className="py-8 text-sm text-muted-foreground">{showInactive ? 'No cancelled or dismissed subscriptions.' : 'No active subscriptions detected. New statement uploads may reveal more.'}</p> :
        <ul className="divide-y">{subscriptions.map(sub => <li key={sub.id} className="flex flex-wrap items-center justify-between gap-3 py-4 first:pt-0">
          <div className="min-w-0 flex-1 basis-36"><h3 className="break-words text-sm font-semibold">{cancellationProvider(sub.service_name)?.name || sub.service_name}</h3><p className="mt-1 text-sm text-muted-foreground">{money(Number(sub.amount))} · {sub.frequency}</p><p className="mt-1 text-xs text-muted-foreground">{money(annualSubscriptionCost(sub))} estimated per year</p></div>
    ...
    <Dialog open={!!selected} onOpenChange={open => { if (!open && !saving) setSelected(null); }}>
      <DialogContent><DialogHeader><DialogTitle>{selected?.status === 'active' ? 'Cancel' : 'Manage'} {selected?.service_name}</DialogTitle><DialogDescription>Casher cannot cancel payments for you. Cancel with the provider first, then update your record here.</DialogDescription></DialogHeader>
        ...
        {provider && <Button asChild variant="outline"><a href={provider!.url} target="_blank" rel="noopener noreferrer">Continue to {provider?.name} cancellation<ExternalLink className="ml-2 h-4 w-4" /></a></Button>}
        ...
        <details><summary className="cursor-pointer py-2 text-sm underline underline-offset-4">Edit detected payment details</summary><form onSubmit={event => { event.preventDefault(); void save(selected?.status || 'active', true); }} className="space-y-3 border-y py-4">
