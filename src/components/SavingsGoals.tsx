...
export default function SavingsGoals({ userId }: { userId: string }) {
  ...
  const query = useQuery({ queryKey: ['savings-goals', userId], retry: retryRead, staleTime: 60_000, queryFn: ({ signal }) => readRequest(async readSignal => {
    const { data, error } = await supabase.from('savings_goals').select('*').eq('user_id', userId).order('created_at', { ascending: false }).abortSignal(readSignal).retry(false);
  ...
  return <div className="min-w-0 space-y-6"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-muted-foreground">Set a target. Make it happen.</p><Button variant="soft" size="sm" onClick={() => showEditor(null)}>Add goal</Button></div>
    <div>{query.isPending ? <p role="status">Loading goals…</p> : query.isError ? <div role="alert"><p>Goals could not be loaded.</p><Button variant="outline" onClick={() => query.refetch()}>Try again</Button></div> : !query.data?.length ? <p className="py-6 text-sm text-muted-foreground">No goals yet. Set a target to start tracking your progress.</p> : <ul className="space-y-5">{query.data.map(goal => {
      const progress = goal.target_amount > 0 ? Math.min(100, Math.max(0, goal.current_amount / goal.target_amount * 100)) : 0;
      return <li key={goal.id} className="rounded-2xl bg-card p-4"><div className="flex flex-wrap justify-between gap-2"><h3 className="min-w-0 max-w-full break-words text-xl font-semibold">{goal.title}</h3><div className="flex gap-2"><Button variant="ghost" size="sm" onClick={() => showEditor(goal)}>Update progress</Button><Button variant="ghost" size="sm" onClick={() => setDeleting(goal)}>Delete</Button></div></div><p className="my-3 text-lg font-semibold">{money(goal.current_amount)} of {money(goal.target_amount)}</p><Progress aria-label={`${goal.title}: ${progress.toFixed(0)}% complete`} value={progress} /><p className="mt-2 text-xs text-muted-foreground">{progress.toFixed(0)}% complete{goal.deadline ? ` · Due ${new Date(goal.deadline + 'T12:00:00').toLocaleDateString('en-GB')}` : ''}</p></li>;
    })}</ul>}</div><p className="app-caption">Progress is recorded by you. Casher does not move or hold your money.</p>
    <Dialog open={open} onOpenChange={value => { if (!busy) setOpen(value); }}><DialogContent><DialogHeader><DialogTitle>{editing ? 'Update savings goal' : 'Create savings goal'}</DialogTitle><DialogDescription>Your saved amount is entered manually.</DialogDescription></DialogHeader><form onSubmit={save} className="space-y-4">
      <div><Label htmlFor="goal-title">Goal title</Label><Input disabled={busy} id="goal-title" value={title} onChange={e => setTitle(e.target.value)} maxLength={120} required /></div>
      <div><Label htmlFor="goal-target">Target amount (£)</Label><Input disabled={busy} id="goal-target" type="number" inputMode="decimal" min="0.01" max="99999999.99" step="0.01" required value={target} onChange={e => setTarget(e.target.value)} /></div>
      <div><Label htmlFor="goal-saved">Already saved (£)</Label><Input disabled={busy} id="goal-saved" type="number" inputMode="decimal" min="0" max="99999999.99" step="0.01" required value={saved} onChange={e => setSaved(e.target.value)} /></div>
      <div><Label htmlFor="goal-deadline">Deadline (optional)</Label><Input disabled={busy} id="goal-deadline" type="date" value={deadline} onChange={e => setDeadline(e.target.value)} /></div>
