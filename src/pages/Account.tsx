...
export default function Account() {
  ...
  const { user, loading } = useAuth(!busy && !deleted);
  ...
  return <div className="min-h-screen bg-background">
    ...
    <header className="border-b px-4 py-4"><Button asChild variant="ghost"><Link to="/dashboard" aria-label="Back to dashboard"><span aria-hidden="true">Back to dashboard</span></Link></Button></header>
    <main className="mx-auto max-w-2xl space-y-8 px-4 py-8">
      <h1 className="text-3xl font-semibold">Account</h1>
      <p className="break-all text-sm text-muted-foreground">{`Signed in as ${user.email}`}</p>
      <section className="space-y-3"><h2 className="text-xl font-semibold">Your data</h2><p className="text-sm text-muted-foreground">Download your profile, imported transactions, corrections, detected subscriptions, goals and upload history as a JSON file. This is available on every plan. Contact privacy@trycasher.com for other personal-data requests.</p><div className="flex flex-wrap gap-3"><Button variant="outline" onClick={exportData} disabled={exporting || busy || clearing}>{exporting ? 'Preparing your data…' : 'Export my saved data'}</Button><Button asChild variant="ghost"><Link to="/dashboard/history" aria-label="Open transaction history"><span aria-hidden="true">Open transaction history</span></Link></Button></div>{exportError && <p role="alert" className="text-sm text-destructive">{exportError}</p>}<nav aria-label="Account policies" className="flex flex-wrap gap-x-5 text-sm"><Link to="/support" className="inline-flex min-h-11 items-center underline">Help and support</Link><Link to="/privacy" className="inline-flex min-h-11 items-center underline" aria-label="Privacy Policy"><span aria-hidden="true">Privacy Policy</span></Link><Link to="/terms" className="inline-flex min-h-11 items-center underline" aria-label="Terms of Service"><span aria-hidden="true">Terms of Service</span></Link></nav></section>
