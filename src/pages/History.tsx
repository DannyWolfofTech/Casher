...
export default function History() {
  ...
  return <div className="min-h-screen">
    ...
    <main id="main-content" className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6">
      <div><Link className="inline-flex min-h-11 items-center text-sm underline underline-offset-4" to="/dashboard" aria-label="Back to overview"><span aria-hidden="true">Back to overview</span></Link><h1 className="mt-4 text-3xl font-semibold tracking-tight">Spending history</h1><p className="mt-2 text-sm text-muted-foreground">Understand the statements you have imported over time.</p></div>
        ...
        <div className="grid min-w-0 gap-6 lg:grid-cols-2">
          <Card className="min-w-0"><CardHeader><CardTitle>Monthly spending</CardTitle><CardDescription>Money out by transaction date, excluding money in. Only imported months are shown.</CardDescription></CardHeader><CardContent>
            <div className="mb-6 grid grid-cols-1 items-end gap-4 sm:grid-cols-2">
              <label className="min-w-0 flex-1 text-xs"><span aria-hidden="true">From month</span><select aria-label="From month" value={from} onChange={e => setFrom(e.target.value)} className="mt-1 block h-11 w-full min-w-0 rounded-md border bg-background px-3 text-base"><option value="">All months</option>{availableMonths(transactions.data || []).map(month => <option key={month} value={month}>{monthLabel(month)}</option>)}</select></label>
              <label className="min-w-0 flex-1 text-xs"><span aria-hidden="true">To month</span><select aria-label="To month" value={to} onChange={e => setTo(e.target.value)} className="mt-1 block h-11 w-full min-w-0 rounded-md border bg-background px-3 text-base"><option value="">All months</option>{availableMonths(transactions.data || []).map(month => <option key={month} value={month}>{monthLabel(month)}</option>)}</select></label>
            ...
            {invalidRange ? <p role="alert">Choose an end month on or after the start month.</p> : trend.length === 0 ? <p className="py-12 text-sm text-muted-foreground">No imported transactions in this period.</p> : <>
              <div className="h-64 w-full min-w-0 overflow-hidden" aria-hidden="true"><ResponsiveContainer width="100%" height="100%" minWidth={0}><BarChart data={trend} margin={{ left: 0, right: 8, bottom: 8 }}>
                <CartesianGrid vertical={false} stroke="hsl(var(--border))" /><XAxis dataKey="key" tickFormatter={key => key.slice(2)} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} /><YAxis width={65} tickFormatter={value => `£${Number(value).toLocaleString('en-GB', { notation: 'compact', maximumFractionDigits: 1 })}`} tick={{ fontSize: 11, fill: 'hsl(var(--muted-foreground))' }} />{!mobile && <Tooltip labelFormatter={key => trend.find(point => point.key === key)?.month || key} formatter={(value: number) => [money(value), 'Money out']} contentStyle={{ background: 'hsl(var(--popover))', color: 'hsl(var(--popover-foreground))', borderColor: 'hsl(var(--border))' }} />}<Bar dataKey="spending" fill="#327a61" radius={[4, 4, 0, 0]} maxBarSize={64} isAnimationActive={false} />
              </BarChart></ResponsiveContainer></div>
              {mobile ? <ul aria-label="Imported spending by transaction month" className="mt-4 divide-y">{trend.map(point => <li key={point.key} className="space-y-2 py-3"><h3 className="text-sm font-medium">{point.month}</h3><dl className="space-y-2 text-sm"><div className="flex flex-wrap justify-between gap-x-3"><dt>Money out</dt><dd className="max-w-full break-all tabular-nums"><AccessibleAmount value={money(point.spending)} label={`Money out: ${money(point.spending)}, ${point.month}.`} /></dd></div><div className="flex flex-wrap justify-between gap-x-3"><dt>Money in</dt><dd className="max-w-full break-all tabular-nums"><AccessibleAmount value={money(point.income)} label={`Money in: ${money(point.income)}, ${point.month}.`} /></dd></div></dl></li>)}</ul> : <div className="overflow-x-auto" tabIndex={0} role="region" aria-label="Monthly amounts, scroll horizontally if needed"><table className="mt-4 w-full text-sm"><caption className="sr-only">Imported spending by transaction month</caption><thead><tr className="border-b"><th className="py-2 text-left font-medium">Month</th><th className="text-right font-medium">Money out</th><th className="text-right font-medium">Money in</th></tr></thead><tbody>{trend.map(point => <tr key={point.key} className="border-b last:border-0"><th scope="row" className="py-3 text-left font-normal">{point.month}</th><td className="text-right tabular-nums"><AccessibleAmount value={money(point.spending)} label={`Money out: ${money(point.spending)}, ${point.month}.`} /></td><td className="text-right tabular-nums"><AccessibleAmount value={money(point.income)} label={`Money in: ${money(point.income)}, ${point.month}.`} /></td></tr>)}</tbody></table></div>}
