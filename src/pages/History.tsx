        ...
        <div className="grid min-w-0 gap-6 lg:grid-cols-2">
          <Card className="min-w-0"><CardHeader><CardTitle>Monthly spending</CardTitle><CardDescription>Money out by transaction date, excluding money in. Only imported months are shown.</CardDescription></CardHeader><CardContent>
            ...
            {invalidRange ? <p role="alert">Choose an end month on or after the start month.</p> : trend.length === 0 ? <p className="py-12 text-sm text-muted-foreground">No imported transactions in this period.</p> : <>
              ...
              {mobile ? <ul aria-label="Imported spending by transaction month" className="mt-4 divide-y">{trend.map(point => <li key={point.key} className="space-y-2 py-3"><h3 className="text-sm font-medium">{point.month}</h3><dl className="space-y-2 text-sm"><div className="flex flex-wrap justify-between gap-x-3"><dt>Money out</dt><dd className="max-w-full break-all tabular-nums">{money(point.spending)}</dd></div><div className="flex flex-wrap justify-between gap-x-3"><dt>Money in</dt><dd className="max-w-full break-all tabular-nums">{money(point.income)}</dd></div></dl></li>)}</ul> : <div className="overflow-x-auto" tabIndex={0} role="region" aria-label="Monthly amounts, scroll horizontally if needed"><table className="mt-4 w-full text-sm"><caption className="sr-only">Imported spending by transaction month</caption><thead><tr className="border-b"><th className="py-2 text-left font-medium">Month</th><th className="text-right font-medium">Money out</th><th className="text-right font-medium">Money in</th></tr></thead><tbody>{trend.map(point => <tr key={point.key} className="border-b last:border-0"><th scope="row" className="py-3 text-left font-normal">{point.month}</th><td className="text-right tabular-nums">{money(point.spending)}</td><td className="text-right tabular-nums">{money(point.income)}</td></tr>)}</tbody></table></div>}
